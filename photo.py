...
class PhotoImageWrapper(Image):
    def __init__(self, parent, image_path: str, is_logo: bool):
        ...
        if not is_logo:
            ...
            self.parent.resize_canvas_event(width=new_width, height=new_height)

            self.img_id = self.parent.create_image(0,
                                                   0,
                                                   anchor=tk.NW,
                                                   image=self.image_tk)
            self.update_original_image(self.original_image)
            self.image_opened_event()
        else:
            resize_quantity = 200
            ...
            self.image_id = self.create_image((canvas_width - new_width) / 2,
                                              (canvas_height - new_height) / 2,
                                              anchor=tk.NW,
                                              image=self.image_tk)

            self.parent.enable_drag_logo()

            self.parent.logo_opened_event()
